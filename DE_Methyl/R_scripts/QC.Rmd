...
# load('meth_kit_atf_vs_lmf.RData')
```
...
```{r}
load('~/R/Fusarium_Flax/DE_Methyl/meth_kit_atf_vs_lmf_chh.RData')
```


```{r echo=FALSE, fig.height=5.5, fig.width=6}
for (i in 1:6) {
  getMethylationStats(myobj.norm[[i]],plot=T,both.strands=FALSE, labels = T)
}
```
...
getCorrelation(meth_3l ,plot=T, method = 'spearman')
```

#### _For uniting 2 replicates - same plot_
```{r, echo=FALSE, warning=FALSE, fig.height=8, fig.width=10}
getCorrelation(meth_2l ,plot=F, method = 'spearman')
```

#### _For uniting 1 replicate - same plot_
```{r, echo=FALSE, warning=FALSE, fig.height=8, fig.width=10}
getCorrelation(meth_1l ,plot=F, method = 'spearman')
```

### Looking through clusterization depending on correlation 

#### _For uniting 3 replicates - same_
```{r echo=FALSE, message=FALSE}
clusterSamples(meth_3l, dist="euclidean", method="ward", plot=TRUE)
```

#### _For uniting 2 replicates - same_
```{r, echo=FALSE, eval=FALSE}
clusterSamples(meth_2l, dist="euclidean", method="ward", plot=TRUE)
```

#### _For uniting 1 replicates -same_
```{r eval=FALSE, echo=FALSE}
clusterSamples(meth_1l, dist="euclidean", method="ward", plot=TRUE)
```
**I think it have no sense, because of its shifted distribution**

### Let's see PCA

### Screeplot
```{r}
PCASamples(meth_3l, screeplot=TRUE)
```
